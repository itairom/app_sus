...
export const bookShopService = {
    ...
    getFromGoogleBooks,
    setFromGoogleBooks,
    _saveGoogleBooksToStorage,
    getGoogleBooks,
    addBook
}

const KEY_BOOKS = 'books'
const KEY_GOOGLE_BOOKS = 'google_books'
let gBooks;
let gGoogleBooks = [];
...
function getFromGoogleBooks(searchVal) {
    return axiosService.askFromGoogleBooks(searchVal)
        .then((books) => {
            return books
        })
        // .then((books) => setFromGoogleBooks(books))
}

function setFromGoogleBooks(books) {
    books.forEach(book => {
        gGoogleBooks.unshift(_createBook(book))
    })
 _saveGoogleBooksToStorage()
}
...
function query(filterBy) {
    if (filterBy) {
        ...
        // maxSpeed = maxPrice ? maxPrice : Infinity
        // minPrice = minPrice ? minPrice : 0
        const filterdBooks = gBooks.filter(book => {
            return book.title.includes(title) // &&book.listPrice < maxPrice //|| book.listPrice > minPrice
        })
...
function addBook(bookId) {
    console.log(gBooks);
    let bookFind = gGoogleBooks.filter(book => {
        return bookId === book.id
    })
    gBooks.unshift(bookFind)
    _saveBooksToStorage
}
...
function getpageCountDescription(pageCount) {
    if (pageCount > 500) {
        return 'Long reading'
    }
...
function _createBook(book) {
    // console.log(book);
    // const { id, title, subtitle, authors, publishedDate, description, pageCount, categories, thumbnail, language, listPrice } = args
    if (book.kind) {
        const { title, authors, subtitle, imageLinks, categories, publishedDate, description, pageCount, language } = book.volumeInfo
        const { id } = book
        return {
            id,
            title,
...
function _createBooks() {
    var books = storageService.loadFromStorage(KEY_BOOKS)
    if (!books || !books.length) {
        books = _getDefaultBooks()
        // books = []
        // for (const book of gBooks) {
...
function _saveGoogleBooksToStorage() {
    storageService.saveToStorage(KEY_GOOGLE_BOOKS, gGoogleBooks)
}

function _getDefaultBooks() {
    return [
        {
            "id": "OXeMG8wNskc",
            ...
            "publishedDate": 1999,
            "description": "Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla, dolorum! Sapiente libero esse atque facere laborum odio. In asperiores saepe ullam dolore. Dolor, qui. Quo voluptatibus nemo, reprehenderit consequuntur assumenda dolorem hic quam et recusandae corporis perferendis laudantium fugiat maxime reiciendis facilis rerum architecto facere temporibus non numquam accusamus. Aspernatur tempore harum cumque eveniet totam distinctio dignissimos modi qui omnis magnam fugiat, commodi quas ducimus est voluptatem ipsa. Sit amet quidem labore reiciendis a obcaecati odio sed? Maxime, explicabo minima cupiditate non earum quis eveniet aliquam adipisci, consequuntur, obcaecati excepturi delectus odit id atque quisquam perferendis voluptatibus nam provident officiis!",
            "pageCount": 713,
            "categories": [
                "Computers",
                "Hack"
